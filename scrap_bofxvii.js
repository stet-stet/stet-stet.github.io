...
async function get_bms_names(page){
    await page.goto("http://manbow.nothing.sh/event/event.cgi?action=sp&event=137",{timeout:120000});
...
async function scrap(number,page){
    ...
    //사람 .entry-c > .entry-title > strong
    const url = `http://manbow.nothing.sh/event/event.cgi?action=More_def&num=${number}&event=137`
    for(let j=0;j<=5;++j){
        try{
...
async function rebuild_database(page){
    ...
    for(let i=0;i<entries;i++){
        ...
        await new Promise(function(resolve){setTimeout(function(){resolve('foo');},1000);}); 
...
async function write_to_file(db){
    fs.writeFileSync('data_bofxvii.json',JSON.stringify(db,null,2),function(error){
